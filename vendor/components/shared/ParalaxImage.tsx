	...
	return (
		<section ref={elementRef} className={`relative ${heightClasses[height]} overflow-hidden`}>
			...
			<div
				...
				<Image src={finalImageUrl} alt={image?.alt || imageAlt} fill className='object-cover object-center' priority />
			</div>
			...
			{(title || subtitle) && (
				<div className={`relative z-10 h-full flex flex-col justify-center px-4 sm:px-6 lg:px-8`}>
					<div
						...
						{title && <h2 className='text-3xl sm:text-4xl lg:text-5xl font-bold mb-6 drop-shadow-lg'>{title}</h2>}
						{subtitle && (
							<p className='text-lg sm:text-xl lg:text-2xl text-orange-1 max-w-2xl leading-relaxed drop-shadow-md'>{subtitle}</p>
						)}
					</div>
				</div>
			)}
