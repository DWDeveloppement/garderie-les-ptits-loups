...
export function DevJsonViewer({ data, slug, title, collapsed = true, isHidden = false }: DevJsonViewerProps) {
  ...
  if (!isHidden) {
    return (
      <div className='fixed right-0 bottom-0 left-0 z-[9999] border-t-4 border-yellow-500 bg-gray-900 text-white shadow-2xl'>
        {/* Header */}
        <div className='flex items-center justify-between border-b border-gray-700 bg-gray-800 px-4 py-2'>
          <div className='flex items-center gap-3'>
            <span className='rounded bg-yellow-500 px-2 py-1 text-xs font-bold text-black'>DEV</span>
            <h3 className='font-mono text-sm font-semibold'>{displayTitle}</h3>
            {slug && <span className='rounded bg-blue-600 px-2 py-0.5 font-mono text-xs'>{slug}</span>}
            <span className='text-xs text-gray-400'>
              ({typeof data === 'object' && data ? Object.keys(data).length : 0} keys)
            </span>
          </div>
