								...
								<Link href={`/${structure.id}`} className='flex h-full' aria-label={`Voir les détails de ${structure.title}`}>
									<Card className='flex flex-col justify-start items-center h-full hover:shadow-lg transition-all duration-300 hover:-translate-y-1 bg-white border-orange-6 hover:border-purple-7 cursor-pointer !p-0'>
										<CardHeader className='flex flex-col justify-start items-center gap-4'>
											...
											<CardDescription className='mb-4 text-purple-9 font-medium text-lg'>{structure.ageRange}</CardDescription>
										</CardHeader>
										...
										<CardFooter className='w-full h-16 flex justify-center items-end'>
											<Button
												className='flex w-full h-full bg-purple-9 group-hover:bg-purple-10 group-hover:scale-105 focus:outline-none focus:ring-2 focus:ring-purple-9 focus:ring-offset-2 cursor-pointer'>
