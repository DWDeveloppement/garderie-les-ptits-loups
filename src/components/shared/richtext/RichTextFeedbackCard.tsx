...
const feedbackCardVariants = cva('relative w-full max-w-2xl mx-auto rounded-xl border overflow-hidden bg-white text-[inherit] shadow-sm', {
	variants: {
		variant: {
...
export function RichTextFeedbackCard({ children, className, icon, title, size, variant }: FeedbackCardProps) {
	...
	const resolvedIcon = React.useMemo(() => {
		...
		if (typeof icon === 'string') {
			...
			return <Icon name={iconName} size={iconSizeByCard[cardSize]} className='text-current' />
		}
	...
	const content = <div className='whitespace-pre-line text-[inherit]'>{children}</div>

	return (
		<div data-slot='feedback-card' className={cn(feedbackCardVariants({ variant, size }), className)}>
			{resolvedIcon ? (
				<div className='flex items-start gap-4'>
					<div className='mt-1 shrink-0 text-[inherit]'>{resolvedIcon}</div>
					<div className='flex flex-col gap-2 text-[inherit]'>
						{title ? <div className='font-semibold whitespace-pre-line text-[inherit]'>{title}</div> : null}
						{content}
					</div>
				</div>
			) : (
