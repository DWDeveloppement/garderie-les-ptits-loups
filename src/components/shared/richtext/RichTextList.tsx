...
import type { PortableTextMarkDef } from '@/types/sanity/core/portableText'
...
type RichTextListProps = VariantProps<typeof listVariants> & {
	...
	renderSpanNodes: (spans: MinimalSpan[], keyPrefix: string, markDefs?: PortableTextMarkDef[]) => ReactNode[]
	className?: string
}
			...
			{items.map((item, index) => {
				...
				return (
					<li key={key} className={listItemVariants({ variant })}>
						{renderSpanNodes(item.children ?? [], key, item.markDefs)}
					</li>
				)
			})}
		</ListTag>
