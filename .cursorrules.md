# 🎯 Règles Cursor - Garderie Les P'tits Loups

## 🇫🇷 Langue de communication

**IMPORTANT** : Tous les échanges, code, commentaires et documentation doivent être **en français**.

---

## 🤖 Optimisation des modèles IA

Pour économiser les tokens et optimiser les coûts, utiliser le modèle approprié :

### Sélection rapide

| Tâche | Modèle | Autorisation |
|-------|--------|--------------|
| 📝 Documentation, typos, lecture | **Haiku 4.5** (gratuit) | ❌ Non |
| 🧩 Composants simples (Button, Card) | **Haiku 4.5** (gratuit) | ❌ Non |
| ⚙️ Formulaires, hooks, API | **Sonnet 4.5** (standard) | ❌ Non |
| 🏗️ Architecture, refactoring majeur | **Opus 4.5** (cerveau) | ✅ **Demander** |

### Processus Opus

```
⚠️ Cette tâche nécessite Opus car :
- [Raison 1]
- [Raison 2]
Estimation : ~X tokens
Puis-je procéder ?
```

**Attendre confirmation** avant de démarrer.

> 📖 **Documentation complète** : Voir [CLAUDE.md](./CLAUDE.md)

---

## 🚀 Stack & Scripts

### Stack Technique

- **Framework** : Next.js 15 + TypeScript + Tailwind CSS v4
- **UI** : Shadcn UI + Radix UI
- **Palette** : Orange (neutre) + Purple (accent)
- **CMS** : Sanity v4.6.1
- **Email** : Resend

### Scripts Essentiels

```bash
npm run dev        # Développement (port 3000)
npm run sanity     # Sanity Studio (port 3333)
npm run refresh    # Clean + restart
npm run build      # Build production
npm run lint       # Vérifier le code
```

---

## 📐 Conventions Rapides

### TypeScript

```typescript
// ✅ Strict mode, type > interface, éviter any
type ButtonProps = {
  variant: 'primary' | 'secondary'
  children: React.ReactNode
}
```

### React

```typescript
// ✅ Server Components par défaut
export default async function Page() {
  const data = await fetchData()
  return <Component data={data} />
}

// ✅ 'use client' uniquement si nécessaire
'use client'
export function InteractiveComponent() {
  const [state, setState] = useState()
}
```

### Tailwind CSS

```typescript
// ✅ Palette : Orange (neutre) + Purple (accent)
<Button className="bg-purple-9 hover:bg-purple-10 text-white" />
<Card className="bg-orange-1 border-orange-6" />

// ✅ Utiliser cn() pour merge
import { cn } from 'lib/utils'
className={cn('base-classes', condition && 'conditional', className)}
```

### Nommage

```typescript
// Composants : PascalCase
ContactForm.tsx

// Fonctions : camelCase
validateEmail()

// Constantes : UPPER_SNAKE_CASE
const MAX_FILE_SIZE = 5000000
```

### Commentaires

```typescript
/**
 * Description de la fonction
 *
 * @param param - Description du paramètre
 * @returns Description du retour
 */
function myFunction(param: string): boolean {
  // Commentaire en français
}
```

---

## 🏗️ Architecture

### Data Fetching : SSG + ISR

```typescript
export const revalidate = 60 // Cache 60 secondes
// + Webhook Sanity pour revalidation instantanée
```

### Structure

```
src/
├── app/              # Pages Next.js
├── components/       # Composants React
│   ├── ui/           # Shadcn UI
│   ├── layout/       # Header, Footer
│   ├── pages/        # Sections de pages
│   ├── shared/       # Réutilisables
│   └── forms/        # Formulaires
├── hooks/            # Hooks personnalisés
├── lib/              # Utilitaires
├── styles/           # CSS (palette, fonts)
└── types/            # Types TypeScript

sanity/
├── schemas/          # Schémas de contenu
├── queries/          # Requêtes GROQ
├── types/            # Types Sanity
└── client.ts         # Client configuré
```

---

## 🔒 Sécurité

```typescript
// 1. Client : Zod validation
const schema = z.object({ email: z.string().email() })

// 2. reCAPTCHA v2 Check
const token = await executeRecaptcha()

// 3. Honeypot Detection
if (formData.website) return error

// 4. Server Validation
const validatedData = schema.parse(body)

// ❌ Jamais de secrets dans le code
// ✅ Toujours utiliser .env.local
```

---

## 📚 Documentation Complète

### Navigation

- **[CLAUDE.md](./CLAUDE.md)** : Guide assistant IA complet
- **[CONTRIBUTING.md](./CONTRIBUTING.md)** : Workflow Git, contributions
- **[docs/README.md](./docs/README.md)** : Index complet (25+ fichiers)

**Documentation technique** (6 catégories) :
- **[architecture/](./docs/architecture/)** : 87 composants, 23 hooks, 22 types
- **[sanity/](./docs/sanity/)** : 22 schémas, 11 queries GROQ, images
- **[features/](./docs/features/)** : Forms, gallery, maps, navigation
- **[performance/](./docs/performance/)** : SEO (metadata, sitemap, structured data)
- **[reference/](./docs/reference/)** : Environment, scripts, troubleshooting
- **[tests/](./docs/tests/)** : A11y, SSR, performance, responsive

---

## ✅ Checklist Avant Commit

- [ ] TypeScript : `npx tsc --noEmit`
- [ ] ESLint : `npm run lint`
- [ ] Build : `npm run build`
- [ ] Types définis
- [ ] Commentaires en français
- [ ] Imports avec aliases (`@/*`)
- [ ] Tailwind uniquement
- [ ] Alt text sur images
- [ ] Pas de secrets

---

## 🐛 Troubleshooting

```bash
# Port occupé
npm run kill:dev

# Cache corrompu
npm run clean && npm run dev

# Build échoue
rm -rf .next node_modules && npm install && npm run build
```

---

**Version** : 2.0.0 (aligné avec CLAUDE.md)
**Maintenu par** : Ricardo Do Vale
