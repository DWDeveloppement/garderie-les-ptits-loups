# 👤 Documentation Client

Documentation destinée aux utilisateurs finaux (client) pour la gestion du contenu du site.

---

## 📋 Contenu

| Fichier | Description |
|---------|-------------|
| **[GUIDE_SANITY.md](./GUIDE_SANITY.md)** | Guide complet Sanity Studio pour éditer le contenu |
| **[TRANSFER_PROJECT.md](./TRANSFER_PROJECT.md)** | Procédure de transfert du projet Sanity |

---

## 🎯 Audience

Ces documents sont destinés à :
- **Client final** : Éditer le contenu du site (textes, images, galeries)
- **Gestionnaire de contenu** : Ajouter/modifier des pages, actualités, etc.
- **Administrateur** : Transfert de propriété du projet Sanity

---

## 🚀 Démarrage Rapide

### Accéder à Sanity Studio

**URL** : `https://garderielesptitsloups.sanity.studio` (ou `http://localhost:3333` en local)

**Compte** : Demander les accès à l'équipe de développement

**Guide** : Voir [GUIDE_SANITY.md](./GUIDE_SANITY.md) pour les instructions complètes

---

### Tâches Courantes

**Modifier une page existante** :
1. Ouvrir Sanity Studio
2. Aller dans "Pages" (menu gauche)
3. Sélectionner la page
4. Modifier le contenu
5. Cliquer "Publish"

**Ajouter des photos à une galerie** :
1. Aller dans "Galeries"
2. Sélectionner la galerie
3. Cliquer "+ Ajouter image"
4. Upload image + remplir alt text (obligatoire)
5. Cliquer "Publish"

**Modifier les infos de contact** :
1. Aller dans "Configuration" → "Footer"
2. Modifier adresse, téléphone, email, horaires
3. Cliquer "Publish"

---

## 📚 Ressources

- **Guide complet** : [GUIDE_SANITY.md](./GUIDE_SANITY.md)
- **Transfert projet** : [TRANSFER_PROJECT.md](./TRANSFER_PROJECT.md)
- **Documentation Sanity** : https://www.sanity.io/docs
- **Support** : contact@garderielesptitsloups.ch

---

**Dernière mise à jour** : 2025-12-03
**Maintenu par** : Ricardo Do Vale
