...
// Requêtes pour les 4 documents d'accordéon (optimisées : frequentationType supprimé, limité à 5 items max)
...
// Requête pour le document Subventions (table) (optimisée : tableSubsidiesInfo supprimé, données via SCHEDULE_QUERY)
...
export async function fetchMonthlyNursery() {
	return sanityFetch<{ accordionItems: SanityAccordionItem[] } | null>(qMonthlyNursery, {}, { tag: 'prices-monthly-nursery' })
}

export async function fetchDailyNursery() {
	return sanityFetch<{ accordionItems: SanityAccordionItem[] } | null>(qDailyNursery, {}, { tag: 'prices-daily-nursery' })
}

export async function fetchMonthlyTG() {
	return sanityFetch<{ accordionItems: SanityAccordionItem[] } | null>(qMonthlyTG, {}, { tag: 'prices-monthly-tg' })
}

export async function fetchDailyTG() {
	return sanityFetch<{ accordionItems: SanityAccordionItem[] } | null>(qDailyTG, {}, { tag: 'prices-daily-tg' })
}
