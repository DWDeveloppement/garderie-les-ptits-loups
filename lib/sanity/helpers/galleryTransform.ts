...
export function transformSanityGalleryToPhotos(
  gallery: SanityGalleryImage[] | null | undefined
): Photo[] {
  ...
  return gallery
    .filter(image => image.asset) // Ne garder que les images valides
    .map((image, index) => transformSanityImageToPhoto(image, index));
}
 ...
 */
export function getOptimalGalleryLayout(
  photoCount: number
