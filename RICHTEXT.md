...
- **RichTextRenderer**
  ...
  - [ ] Support futur des images rich-text (figure + classes position/size)
- **Paragraphes**
...
- **Futurs composants**
  - [ ]`RichTextInfoCard` (variants note/info/warning/success/tip)
...
- **Schémas**
  ...
  - [ ] Ajouter un bloc`infoCard` (fields: title, content, variant select, icon optionnel)
  - [ ] Documenter la convention temporaire`{variant:secondary}` si utilisée avant le schema (optionnel)
- **Queries GROQ**
  - [ ] Projeter`quoteEnhanced` dans`ABOUT_QUERY` (text, author, variant)
  - [ ] Projeter`infoCard` (title, content, variant, icon)
- **Studio UX**
